    ...
    onTouchMove: function onTouchMove(event, self) {
        ...
        if (self._lastPai != null) {
            ...
            if (self.moveDirection === 0) {
                ...
                if (delta.x > 0 && self._paiSprites[row][self.moveEnd].x + delta.x <= self.moveMax * self.paiWidth || delta.x < 0 && self._paiSprites[row][self.moveStart].x + delta.x >= self.moveMin * self.paiWidth) {
                    for (var j = self.moveStart; j <= self.moveEnd; j++) {
                ...
                if (delta.y > 0 && self._paiSprites[self.moveEnd][column].y + delta.y <= self.moveMax * self.paiHeight || delta.y < 0 && self._paiSprites[self.moveStart][column].y + delta.y >= self.moveMin * self.paiHeight) {
                    for (var i = self.moveStart; i <= self.moveEnd; i++) {
